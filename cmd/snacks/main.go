...
func helpText(msg string) {
	...
	fmt.Println("  http://locallhost:8080/vulnerable/endpoint")
	fmt.Println()
	fmt.Println("When not specified in the URL the below defaults are used")
	...
	fmt.Println()
	fmt.Println("Notes:")
	fmt.Println("  Slows sends are performed by sending separate frames/packets to a target for")
	fmt.Println("  each segment. Slow sends within a frame/packet are unsuppported as this would")
	fmt.Println("  require a custom TCP IP stack implementation.")
}
